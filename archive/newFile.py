...
def newFile(filename, ext):
    """ Create a new python bash script file with
    the given file name. The filename is either the
    full path or the relative path where you want the
    template"""
    global TEMPLATE_DIR
...
def touchFile(filename):
    """This will update the date and names inside the file
    with the new filename
    TODO: I need another function to make all the comment
    sections look the same"""
    f = open(filename, 'r+')
