...
async def download(links, save_path, loop):
    ...
    async with aiohttp.ClientSession(loop=loop) as session:
        return await asyncio.wait(
            list(map(partial(download_file, save_path, session), links))
        )
...
async def get_inputs(input_filename):
    ...
    with open(input_filename, newline="") as csvfile:
        ...
        for row in reader:
            i = Input(
                row["download_from"].strip(), row["save_to"].strip(), row["has"].strip()
            )
            lines.append(i)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "download_from", help="The URL to download from", nargs="?", default=None
    )
    parser.add_argument(
        "save_to", help="The directory to save to", nargs="?", default=None
    )
    parser.add_argument(
        "--has", help="Regex used for filtering. Files matching are kept"
    )
    parser.add_argument(
        "-i", "--input", help="File to get inputs", nargs="?", default=None
    )
