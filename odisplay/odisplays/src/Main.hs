...
filterScripts scripts = do
  inshell (pipeToRofi scripts) empty
 where
  listToText xs = DT.pack $ concat $ L.intersperse "\n" $ encodeString <$> xs
  pipeToRofi names = "echo \"" <> listToText names <> "\" | rofi -dmenu -i -p 'oDisplays'"

runLayoutScript :: Line -> Shell ()
runLayoutScript s = shells (lineToText s) empty
...
main = sh $ do
  ...
  case scripts of
    ...
    xs  -> (filterScripts xs) >>= runLayoutScript
