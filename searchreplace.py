...
def main():
    ...
    parser.add_argument('-d', '--directory', action='store', default=".",
                    help="The directory to search within, default is current directory")
    ...
    parser.add_argument('-r', '--recursive', action='store_true',
                    help="When searching in a directory, where to decend into sub directories")
...
def search_replace(searchterm,replaceterm,filename):
    if os.path.isfile(filename):
        ...
        for line in fileinput.input(filename, inplace = True):
            if re.match(r''+searchterm, line)!= None:
